...
import "github.com/mohitvermax/go-interpreter/token"

type Lexer struct {
	input        string
	position     int  // curr position in input (points to current char)
	readPosition int  // curr reading position in input (after current char)
...
We only want to “peek” ahead in the input and not move around in it, so
